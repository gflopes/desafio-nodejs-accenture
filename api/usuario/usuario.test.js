...
describe('GET /usuario/:id', function() {
  it('teste da rota de busca de usuário, busca ok, status code esperado => 200', function(done) {
    request(app)
      ...
      .set(
        'authorization',
        'Bearer eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJlbWFpbCI6InRlc3RlNTVAZW1haWwuY29tLmJyIiwiaWF0IjoxNTc1NzI5NDY0LCJleHAiOjE1NzYzMzQyNjR9.PE5HiGqjOC8vG2wx-c9omi_eh6pePmLmz_hecAIjNZc'
      )
      .expect(200)
      .end(function(err, res) {
        if (err) return done(err)
...
describe('GET /usuario/:id', function() {
  it('teste da rota de busca de usuário, token / id usuário não encontrado, status code esperado => 401', function(done) {
    request(app)
      ...
      .set(
        'authorization',
        'Bearer ZZJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJlbWFpbCI6InRlc3RlNTlAZW1haWwuY29tLmJyIiwiaWF0IjoxNTc1NzMwNzQ2LCJleHAiOjE1NzYzMzU1NDZ9.3_95iInDZj_H3Bu9XVYW7qPGA0_ogSfIEzyLOLpmuIs'
      )
      .expect(401)
      .end(function(err, res) {
        if (err) return done(err)
